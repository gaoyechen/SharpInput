...
> 五阶段深度思维框架，让你的 AI 回答从"正确废话"变成"有洞察力的观点"。

大多数人的提问方式，注定了 AI 只能给出中庸答案。SharpAsk 通过 **问题重构 → 认知施压 → 多路径生成 → 自我对抗 → 收敛输出** 五个阶段，强制 AI 产生立场、权衡和洞察，而非罗列教科书常识。
...
>
> ⚔️ **自我对抗：**
> - 最脆弱假设：假设"通过考试"等于"学会知识"
> - 反例：很多架构师考试高分者并不具备实际架构能力
> - 失效条件：如果目标是提升实际能力而非拿证，整个问题需要重构
...
cp -r SharpAsk ~/.workbuddy/skills/
```
...
| 模块 | 内容 |
|------|------|
| 🎯 **认知施压版问题** | 注入立场约束 + 反共识约束 + 权衡约束的优化问题，可直接复制使用 |
| 🔄 **多路径对比** | 2-3 个不同思路的优化版本，附共识度评分（🟢/🟡/🔴） |
| ⚔️ **自我对抗** | 每个路径的最脆弱假设、反例、失效条件 |
| 🔭 **隐藏维度** | 你可能没想到但至关重要的 2+ 个维度 |
| ✅ **最终推荐** | 一个明确的推荐方案 + 适用边界 + 风险提示 |
...
```
Stage 1: 问题重构 (内化)     ──→ 防止垃圾输入
    ↓
Stage 2: 认知施压 🔥         ──→ 逼 AI 产生"观点"
    ↓
Stage 3: 多路径生成          ──→ 避免单一答案
    ↓
Stage 4: 自我对抗 🔥         ──→ 打掉幻觉
    ↓
Stage 5: 收敛输出            ──→ 可执行结论
