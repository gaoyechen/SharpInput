...
大多数人的提问方式，注定了 AI 只能给出中庸答案。SharpAsk 通过 **场景判断 → 问题重构 → 认知施压 → 多路径生成 → 自我对抗 → 收敛输出**，强制 AI 产生立场、权衡和洞察，而非罗列教科书常识。
...
| **Level 2** | 中度施压 | 问题重构 → 认知施压 | 需要有立场，但不需要完整对抗 |
| **Level 3** | 深度对抗 | 完整五阶段 | 复杂决策、需要全面分析 |

**自动分流**：查资料、Debug、翻译等简单问题直接回答，不走 SharpAsk 流程，零摩擦。

**随时切换**：说"深度模式"升级到 Level 3，说"简单优化"降到 Level 1。
...
施压等级选择
    ├── Level 1: 问题重构 ──→ 输出优化问题
    ├── Level 2: 问题重构 → 认知施压 ──→ 输出施压版问题 + 隐藏维度
    └── Level 3: 问题重构 → 认知施压 → 多路径 → 自我对抗 → 收敛输出
                                                                    ↓
                                                              最终推荐
```
...
```
SharpAsk/
├── SKILL.md                         # 主文件：流程 + Gate + 三级施压 + 输出模板
├── references/
