...
| **意图识别** | 判定核心意图（理解/决策/生成/分析/探索） |
| **上下文补全** | 信息不足时提问，信息充足直接进入优化 |
...
| **Intent Recognition** | Classify core intent (Explain / Decision / Generate / Analyze / Explore) |
| **Context Completion** | Ask if info missing, proceed if sufficient |
...
```
Input ≤ 15 chars and no clear question structure?
...
Involves choices / trade-offs / multi-factor analysis?
...
| **Decision** | "Should I", "A or B", "Which to choose" | Regret pre-mortem: what will you regret most in 3 years? |
| **Generate** | "Help me build", "Design", "Write" | Minimal viable: keep only one element — which one? |
| **Analyze** | "Analyze", "Evaluate", "Review this plan" | Hidden assumptions: how does the conclusion change if the premise is wrong? |
