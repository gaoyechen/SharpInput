...
>
> A cognitive forcing framework that sharpens any user input — questions, statements, plans, ideas, requirements — into versions that force AI into producing genuine opinions, real trade-offs, and actionable insight.
...
| 输入类型 | 示例 | Gate 处理方式 |
|---------|------|-------------|
| 🆚 **对比分析** | "React和Vue哪个好" | 进入优化流程 |
| 🤔 **决策判断** | "我该不该跳槽" | 进入优化流程 |
| 🔬 **方案评估** | "这个架构怎么样" | 进入优化流程 |
| 🧭 **方向探索** | "还有什么方向" | 进入优化流程 |
| 📋 **战略规划** | "怎么规划团队扩张" | 进入优化流程 |
| 📊 **问题陈述** | "我们的转化率一直在下降" | 进入优化流程 |
| 🔨 **需求描述** | "帮我设计一个用户增长方案" | 进入优化流程 |
| 💡 **想法验证** | "我觉得应该用微服务重构" | 进入优化流程 |
| 📝 **方案评审** | "这是我的技术方案，帮我看看" | 进入优化流程 |
| 📚 **事实查询** | "闭包是什么" | 直接回答，跳过优化 |
| 🐛 **调试报错** | "这段代码报错了" | 直接回答，跳过优化 |
| 🔄 **格式转换** | "翻译成英文" | 直接回答，跳过优化 |
...
| ⚖️ **Decision** | 做出选择/判断 | 提问 / 需要权衡评估的想法 | 后悔预判 + 杀手问题 |
| 🔨 **Generate** | 生成代码/内容/方案 | 提问 / 需求描述 | 最简方案 + 约束挑战 |
| 🔬 **Analyze** | 分析/评估/审查 | 提问 / 方案评审 / 想法验证 / 问题诊断 | 隐含假设暴露 + 失效条件 |
...
1. **自动推断** — 从输入中提取隐含的背景/目标/场景
2. **展示确认** — "我推测你是 [X]，想解决 [Y]，在 [Z] 场景下，对吗？"
3. **用户修正** — 只需说"对"或纠正一个点

## 🔄 多路径交互选择（Level 3）

Level 3 深度对抗模式生成 2-3 条不同角度的优化路径，用户可以选择、组合或描述偏好：

| 角度标签 | 含义 | 适用意图 |
|---------|------|---------|
| `risk-first` | 聚焦风险和下行分析 | Decision, Analyze |
| `constraint-challenge` | 挑战输入本身的预设 | Explain, Analyze |
| `counter-intuitive` | 寻找反直觉答案 | Explain, Explore |
| `minimalist` | 精简到最核心要素 | Generate, Decision |
| `adversarial` | 站在对立面辩护 | Explore, Decision |
| `time-horizon` | 拉到 3-5 年后审视 | Strategy, Decision |
| `role-reversal` | 从意想不到的视角切入 | All |

- **单选** — 选 A/B/C 中的一条
- **组合** — 最多 3 条路径的元素混搭（如"A的角度 + B的约束"）
- **描述偏好** — "更偏风险视角" → 自动匹配最佳路径

## 🧠 自我学习系统

SharpInput 会记住你的偏好，越用越精准：

- **全 Level 记录** — 不只是 Level 3，Level 0/1/2 的选择也会被学习
...
> 告诉我：如果按你的诊断做了调整，**3 个月后最可能后悔的是什么？**
...
**SharpInput 输出（Level 3，多路径交互）：**

| 路径 | 角度 | 优化后的问题 | 可信度 |
|------|------|------------|--------|
| A | risk-first | "假设微服务重构会在 6 个月内让团队产出降低 40%，你还坚持吗？..." | ⭐⭐⭐⭐⭐ |
| B | counter-intuitive | "说服我为什么不该用微服务——列出 3 个你的单体架构其实比微服务更适合的理由..." | ⭐⭐⭐⭐ |
| C | time-horizon | "站在 3 年后回看，微服务重构可能带来的最大技术债务是什么？..." | ⭐⭐⭐ |
...
```
用户输入 → 🚦 Gate 判断 → 🧠 记忆加载 → 🏷️ 意图识别 → 🔮 上下文补全 → 施压等级选择
    ├── Level 0: 仅认知施压
    ├── Level 1: 问题重构
    ├── Level 2: 认知施压 + 深度施压层
...
Most people provide inputs in ways that guarantee mediocre answers. SharpInput applies **scene gating → memory load → intent recognition → context completion → problem reframing → cognitive forcing → divergent thinking → adversarial loop → convergent synthesis → preference learning** to force AI into producing answers with genuine positions, real trade-offs, and actionable insight — not textbook bullet points.
...
SharpInput doesn't help you "word things better." It:
...
## 🏷️ Covered Input Types

| Input Type | Example | Gate Handling |
|-----------|---------|--------------|
| 🆚 **Comparison** | "React vs Vue" | Enters optimization |
| 🤔 **Decision** | "Should I change jobs" | Enters optimization |
| 🔬 **Analysis** | "How is this architecture" | Enters optimization |
| 🧭 **Exploration** | "What directions are there" | Enters optimization |
| 📋 **Strategy** | "How to scale my team" | Enters optimization |
| 📊 **Problem statement** | "Our conversion rate keeps declining" | Enters optimization |
| 🔨 **Requirement** | "I need a user growth strategy" | Enters optimization |
| 💡 **Idea validation** | "I think we should refactor to microservices" | Enters optimization |
| 📝 **Plan review** | "Here's my technical plan, review it" | Enters optimization |
| 📚 **Factual lookup** | "What is a closure in JS" | Direct answer, skip optimization |
| 🐛 **Debug/error** | "This code throws an error" | Direct answer, skip optimization |
| 🔄 **Formatting** | "Translate to English" | Direct answer, skip optimization |
...
| ⚖️ **Decision** | Make a choice / judgment | Questions / Ideas requiring trade-off evaluation | Regret pre-mortem + killer question |
| 🔨 **Generate** | Generate code / content / solution | Questions / Requirements: "I need a..." | Minimal viable solution + constraint challenge |
| 🔬 **Analyze** | Analyze / evaluate / review | Questions / Plan reviews, idea validation, problem diagnosis | Hidden assumption exposure + failure conditions |
...
1. **Auto-infers** — Extracts implicit background / goals / scenario from the input
...
Level 3 generates 2-3 optimized paths from different angles. Users can select, combine, or describe a preference:

| Angle Tag | Meaning | Best For |
|-----------|---------|----------|
| `risk-first` | Focus on downside analysis | Decision, Analyze |
| `constraint-challenge` | Challenge the input's own premises | Explain, Analyze |
| `counter-intuitive` | Seek the non-obvious answer | Explain, Explore |
| `minimalist` | Strip to essentials | Generate, Decision |
| `adversarial` | Argue against the obvious | Explore, Decision |
| `time-horizon` | Shift to 3-5 years out | Strategy, Decision |
| `role-reversal` | Unexpected stakeholder perspective | All |

- **Single pick** — Choose path A, B, or C
- **Combine** — Mix elements from up to 3 paths (e.g., "A's angle + B's constraint")
- **Describe preference** — "More risk-focused" → auto-matches the best path

## 🧠 Self-Learning System

SharpInput remembers your preferences and improves over time:

- **All-level tracking** — Not just Level 3; Level 0/1/2 choices are also learned
...
### Example 1: Problem Statement → Optimized
...
> (👆 Generic advice any AI would give)
...
### Example 2: Idea Validation → Optimized (Multi-Path)

**Original:**
> I think we should refactor our monolith to microservices.

**SharpInput output (Level 3, multi-path interactive):**

| Path | Angle | Optimized Question | Credibility |
|------|-------|-------------------|-------------|
| A | risk-first | "Assume microservice refactoring will reduce team output by 40% for 6 months. Still want to do it?..." | ⭐⭐⭐⭐⭐ |
| B | counter-intuitive | "Convince me why you should NOT use microservices — list 3 reasons your monolith is actually better..." | ⭐⭐⭐⭐ |
| C | time-horizon | "Looking back from 3 years in the future, what's the biggest tech debt microservices might create?..." | ⭐⭐⭐ |

> 💡 If undecided: Path A has the highest credibility (⭐⭐⭐⭐⭐) and broadest applicability.

### Example 3: Requirement → Optimized
...
```
User input → 🚦 Gate → 🧠 Memory Load → 🏷️ Intent Recognition → 🔮 Context Completion → Forcing Level
