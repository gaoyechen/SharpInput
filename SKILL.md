...
1. **选维**：根据意图类型 + 输入内容，从池中选 2-3 个最切题的维度
   ...
   **批判兜底**：每条路径至少含 1 个批判挑战类维度（counter-intuitive / adversarial / hidden-assumption / risk-first）。默认映射已满足则无需动作，不满足则替换最弱维度。
...
11. 方向确认 — Level ≥ 2 在 Stage 1→2 间必须确认重构方向，避免后续白做
