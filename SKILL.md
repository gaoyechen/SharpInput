...
description: >
  ...
  通过意图识别(14类)、认知施压(14种策略)、反共识检测、角色预设注入（领域专家/魔鬼代言人/跨界猎人），产出可直接复制使用的施压版问题。
...
> **v2.1** — 意图识别重构（14类意图 + 口语关键词 + 语义模式 + 意图驱动上下文补全 + 施压策略矩阵）+ Gate 中间级别 + 维度池扩展 + 角色预设注入
...
| 探索 | 跨界猎人 | 探索需要跳出本领域 |
| 诊断 | 领域专家 | 诊断需要见过类似的坑 |
| 说服 | 魔鬼代言人 | 说服需要预判对方的反驳 |
| 验证 | 魔鬼代言人 | 验证就是找漏洞 |
| 规划 | 魔鬼代言人 | 规划最怕路径依赖 |
| 学习 | 领域专家 | 学习需要知道哪些弯路不用走 |
| 优化 | 魔鬼代言人 | 优化需要质疑"为什么这样不行" |
| 对比 | 跨界猎人 | 对比需要发现被忽视的差异维度 |
| 梳理 | 领域专家 | 梳理需要知道结构中的关键节点 |
| 求助 | 领域专家 | 求助需要见过同类困境的解法 |
...
```
[请以{角色名}的视角回答以下问题，你深耕该领域10年以上，见过所有坑和反直觉的真相/专职挑战主流观点/擅长从其他领域搬运被忽视的模式]
...
- 用户说「用领域专家/魔鬼代言人/跨界猎人」→ 立即切换到指定角色
