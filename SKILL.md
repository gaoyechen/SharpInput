...
### Step 0 — 轻量自检（Level 1-2 内部执行，不展示；Level 3 跳过，由 Judge 覆盖）
...
费曼验证的输出不展示给用户，仅作为内部门控。通过后才进入最终输出环节。
...
9. 费曼门控 — 所有问题必须通过可理解性验证才能输出，不展示验证过程
10. 轻量自检 — Level 1-2 输出前必须自检脆弱假设，致命假设触发重构（最多 1 次）
